...
const ALL_CARS = [
  ...
  },
  {
    id: 'f1',
    name: 'F1 Lightning',
    desc: 'Pure speed machine — front/rear wings, open wheels, DRS ready.',
    stats: { speed: 95, accel: 80, handling: 75, boost: 60 },
    colors: { body: '#e10600', accent: '#ff4444', trim: '#ffffff', wing: '#222' },
    draw: function(ctx, w, h, angle, frame) {
      const bw = w * 0.85, bh = h * 0.42;
      ...
      ctx.beginPath();
      ctx.ellipse(3, 5, bw/2 + 2, bh/2 + 2, 0, 0, Math.PI*2);
      ctx.fill();

      // Rear wing
      ctx.fillStyle = this.colors.wing;
      ctx.fillRect(-bw*0.44, -bh*0.55, bw*0.12, bh*1.1);
      ctx.fillStyle = this.colors.trim;
      ctx.fillRect(-bw*0.46, -bh*0.58, bw*0.16, 3);

      // Nose cone (long, pointed front)
      ctx.fillStyle = this.colors.body;
      ctx.beginPath();
      ctx.moveTo(bw*0.48, 0);
      ctx.lineTo(bw*0.3, -bh*0.18);
      ctx.lineTo(-bw*0.2, -bh*0.22);
      ctx.lineTo(-bw*0.35, -bh*0.18);
      ctx.lineTo(-bw*0.35, bh*0.18);
      ctx.lineTo(-bw*0.2, bh*0.22);
      ctx.lineTo(bw*0.3, bh*0.18);
      ctx.closePath();
      ...
      ctx.fillStyle = this.colors.accent;
      ctx.beginPath();
      ctx.ellipse(-bw*0.05, -bh*0.28, bw*0.12, bh*0.1, 0, 0, Math.PI*2);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(-bw*0.05, bh*0.28, bw*0.12, bh*0.1, 0, 0, Math.PI*2);
      ctx.fill();

      // Halo protection (cockpit surround)
      ctx.strokeStyle = '#666';
      ctx.lineWidth = 2.5;
      ctx.beginPath();
      ctx.arc(bw*0.15, 0, bh*0.2, -Math.PI*0.6, Math.PI*0.6);
      ctx.stroke();

      // Cockpit opening
      ctx.fillStyle = 'rgba(20,20,30,0.8)';
      ctx.beginPath();
      ctx.ellipse(bw*0.18, 0, bw*0.08, bh*0.14, 0, 0, Math.PI*2);
      ctx.fill();

      // Driver helmet
      ctx.fillStyle = this.colors.trim;
      ctx.beginPath();
      ctx.arc(bw*0.2, 0, bh*0.08, 0, Math.PI*2);
      ctx.fill();
      ctx.fillStyle = this.colors.body;
      ctx.beginPath();
      ctx.arc(bw*0.21, -bh*0.02, bh*0.05, 0, Math.PI*2);
      ctx.fill();
      ...
      ctx.fillText('1', bw*0.35, 0);

      // Front wing
      ctx.fillStyle = this.colors.wing;
      ctx.fillRect(bw*0.36, -bh*0.35, bw*0.1, bh*0.7);
      ctx.fillStyle = this.colors.trim;
      ctx.fillRect(bw*0.36, -bh*0.38, bw*0.12, 3);
      ctx.fillRect(bw*0.36, bh*0.35, bw*0.12, 3);

      // Exhaust flame (animated)
      const flameLen = 6 + Math.sin(frame * 0.3) * 3;
      ctx.fillStyle = 'rgba(255, 180, 0, 0.7)';
      ctx.beginPath();
      ctx.moveTo(-bw*0.38, -bh*0.06);
      ctx.lineTo(-bw*0.38 - flameLen, 0);
      ctx.lineTo(-bw*0.38, bh*0.06);
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = 'rgba(255, 100, 0, 0.5)';
      ctx.beginPath();
      ctx.moveTo(-bw*0.38, -bh*0.03);
      ctx.lineTo(-bw*0.38 - flameLen * 1.4, 0);
      ctx.lineTo(-bw*0.38, bh*0.03);
      ctx.closePath();
      ctx.fill();

      // Open wheels (4 exposed tires)
      ctx.fillStyle = '#1a1a1a';
      const wheelW = 5, wheelH = 8;
      ctx.beginPath();
      ctx.ellipse(bw*0.35, -bh*0.42, wheelW, wheelH, 0, 0, Math.PI*2);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(bw*0.35, bh*0.42, wheelW, wheelH, 0, 0, Math.PI*2);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(-bw*0.3, -bh*0.42, wheelW, wheelH, 0, 0, Math.PI*2);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(-bw*0.3, bh*0.42, wheelW, wheelH, 0, 0, Math.PI*2);
      ctx.fill();
      ...
      ctx.strokeStyle = this.colors.trim;
      ctx.lineWidth = 1;
      for (const [wx, wy] of [[bw*0.35, -bh*0.42], [bw*0.35, bh*0.42], [-bw*0.3, -bh*0.42], [-bw*0.3, bh*0.42]]) {
        ctx.beginPath();
        ctx.ellipse(wx, wy, wheelW + 1, wheelH + 1, 0, 0, Math.PI*2);
        ctx.stroke();
      }

      // T-cam (overhead camera pod)
      ctx.fillStyle = this.colors.body;
      ctx.fillRect(bw*0.14, -bh*0.08, 4, bh*0.16);
